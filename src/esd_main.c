...
static bool __esd_check_valid_privilege_by_db(uid_t uid, const char *appid, const char *privilege_name)
{
	int ret = 0;
	char *pkgid = NULL;
	bool has_privilege = false;
	pkgmgrinfo_pkginfo_h handle = NULL;
	esd_privilege_check *epc = NULL;

	epc = calloc(1, sizeof(esd_privilege_check));
	if (epc == NULL) {
		_E("memeory alloc failed");
		return ES_R_ENOMEM;
	}

	if (__esd_get_pkgid_by_appid(uid, appid, &pkgid) < 0) {
		return ES_R_ERROR;
	}
	...
	epc->privilege_name = privilege_name;
	ret = pkgmgrinfo_pkginfo_foreach_privilege(handle, __esd_privilege_func, epc);
	if (ret != PMINFO_R_OK) {
...
static int __esd_check_app_privileged_event(uid_t uid, const char *appid, const char *event_name)
{
	...
	_D("event_name(%s), uid(%d), appid(%s)", event_name, uid, appid);

	__esd_check_privilege_name(event_name, &privilege_name);

	if (privilege_name && !__esd_check_valid_privilege_by_db(uid, appid, privilege_name)) {
